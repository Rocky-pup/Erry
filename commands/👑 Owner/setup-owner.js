        ...
        client.on('interactionCreate',  (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content : `:no_entry_sign: You already selected something, this Selection is now disabled!`, ephermal : true});
              menuselection(menu);
            }
            else menu?.reply({content : `:no_entry_sign: You are not allowed to do that! Only: <@${cmduser.id}>`, ephermal : true});
          }
        });
      }
            ...
            await tempmsg.channel.awaitMessages({filter: m => m.author.id === message.author.id,
              ...
              .then(collected => {
                ...
                if (user) {
                  ...
                  if(user.id == "913117505541775420"){
                    return message.channel.send({embeds: [new MessageEmbed()
                      .setFooter(client.getFooter(es))
                      .setColor(es.wrongcolor)
                      .setTitle(`You can't remove Rocky!`)
