...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      embed.addField("**<:arrow:950884679114952715> Rules Channel:**", `${message.guild.rulesChannel ? `<#${message.guild.rulesChannelId}>`: ":no_entry_sign: \`No Channel\`"}`, true)
      embed.addField("**<:arrow:950884679114952715> Public Updates Channel:**", `${message.guild.publicUpdatesChannel ? `<#${message.guild.publicUpdatesChannelId}>`: ":no_entry_sign: \`No Channel\`"}`, true)
      embed.addField("**<:arrow:950884679114952715> AFK Channel:**", `${message.guild.afkChannel ? `<#${message.guild.afkChannelId}>`: ":no_entry_sign: \`No Channel\`"}`, true)
      ...
      if(boosts >= 14){
          embed.addField(`**<:arrow:950884679114952715> Vanity:**`, `${message.guild.vanityURLCode ? `https://discord.gg/${message.guild.vanityURLCode}` : ":no_entry_sign: No Vanity-Invite"}`)
      }
