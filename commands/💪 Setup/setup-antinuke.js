...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    //only allow the Server owner, (&Rocky) to execute this Command, (Rocky just because if he needs to help for Bot)
      ...
      async function first_layer(){
        ...
        async function menuselection(menu) {
          ...
          if(menu?.values[0] == "Information"){
                    ...
                    async function first_layer(){
                      ...
                      client.on('interactionCreate',  (menu) => {
                        if (menu?.message.id === menumsg.id) {
                          if (menu?.user.id === cmduser.id) {
                            if(used33) return menu?.reply({content: `:no_entry_sign: You already selected something, this Selection is now disabled!`, ephemeral: true})
            ...
            for(const ch of channels.map(this_Code_is_by_Rocky => this_Code_is_by_Rocky)){
              try {
                if(ch) {
