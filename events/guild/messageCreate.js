...
module.exports = async (client, message) => {
  try {
    ...
    if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADD_REACTIONS))
      return message.reply(`:no_entry_sign: **I am missing the Permission to ADD REACTIONS**`).catch(()=>{})
    ...
    if (command && !customcmd) {
      ...
      if(musicData.channel && musicData.channel == message.channel.id){
        return message.reply(":no_entry_sign: **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)}).catch(()=>{})
      }
      if (command.length == 0) {
        if (unkowncmdmessage) {
