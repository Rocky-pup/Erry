...
function GetUser(message, arg){
  var errormessage = ":no_entry_sign: I failed finding that User...";
...
function GetRole(message, arg){
  var errormessage = ":no_entry_sign: I failed finding that Role...";
  return new Promise(async (resolve, reject) => {
    var args = arg, client = message.client;
    if(!client || !message) return reject("CLIENT IS NOT DEFINED")
    if(!args || args == null || args == undefined) args = message.content.trim().split(/ +/).slice(1);
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if(!user && args[0] && args[0].length == 18) {
      ...
      for(const g of guilds){
        var members = g.members.cache.map(this_Code_is_by_Rocky => this_Code_is_by_Rocky);
      ...
      if (data.rosterstyle == "1") {
        ...
        for (let i = 0; i < memberarray.length; i += 20) {
          ...
          if (!the_roster_db?.get(guild.id, pre+".showallroles") || memberarray.length < 20)
            try { 
              rosterembed.addField(`**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_Rocky => this_Code_is_by_Rocky).length}]__**`, role.members.map(this_Code_is_by_Rocky => this_Code_is_by_Rocky).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substring(0, leftnum <= 1024 ? leftnum : 1024) + `${role.members.map(this_Code_is_by_Rocky => this_Code_is_by_Rocky).length - 20 > 0 ? `\n${the_roster_db?.get(guild.id, pre+".rosteremoji")} ***\`${role.members.map(this_Code_is_by_Rocky => this_Code_is_by_Rocky).length - 20}\` other Members have this Role ...***`: ""}`.substring(0, 1024), the_roster_db?.get(guild.id, pre+".inline"))
              break;
            } catch (e) {
              console.error(e)
            }
            ...
            try {
              rosterembed.addField(i < 20 ? `**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_Rocky => this_Code_is_by_Rocky).length}]__**` : `\u200b`, role.members.map(this_Code_is_by_Rocky => this_Code_is_by_Rocky).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substring(0, leftnum <= 1024 ? leftnum : 1024), the_roster_db?.get(guild.id, pre+".inline"))
            } catch (e) {
              console.error(e)
            }
        }
        ...
        await msgs.forEach(async msg => {
            ...
            if(msg.type == "DEFAULT"){
              ...
              if (msg.reactions && msg.reactions.cache.size > 0){
                ...
                for(const reaction of msg.reactions.cache.map(this_Code_is_by_Rocky => this_Code_is_by_Rocky)){                      
...
async function swap_pages(client, message, description, TITLE) {
  ...
  collector.on('collect', async b => {
      if(b?.user.id !== message.author.id)
        return b?.reply({content: `:no_entry_sign: **Only the one who typed ${prefix}help is allowed to react!**`, ephemeral: true})
        //page forward
        if(b?.customId == "1") {
  ...
  for (let i = 0; i < guilds.length; i++) {
      try {
          ...
          await guild.channels.cache.filter(ch => ch.type == "GUILD_VOICE" && jointocreate.includes(ch.id)).each(async (channel, j) => {
              try{
                  let members = channel.members.map(this_Code_is_by_Rocky => this_Code_is_by_Rocky);
            ...
            guild.channels.cache.filter(ch => ch.type == "GUILD_VOICE").each(async vc => {
              try{
                  if(client.jointocreatemap.get(`tempvoicechannel_${vc.guild.id}_${vc.id}`) == vc.id){
                      let members = vc.members.map(this_Code_is_by_Rocky => this_Code_is_by_Rocky);
...
async function create_transcript(message, client, msglimit) {
  ...
  let msgs = messageCollection.map(this_Code_is_by_Rocky => this_Code_is_by_Rocky).reverse(); //reverse the array to have it listed like the discord chat
