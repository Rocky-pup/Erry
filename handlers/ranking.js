...
module.exports = async function (client) {
    ...
    module.exports.messageCreate = async (client, message, guild_settings, setups) => {
        try{
            ...
            async function LEVELUP(thekey) {
                ...
                const filtered = await client.points.all().then(d => d.filter(p => p.data?.guild === message.guild.id).map(this_Code_is_by_Rocky => this_Code_is_by_Rocky.data));
    ...
    */
            ...
            async function leaderboardembed(type = "text") {
                ...
                const filtered = await client.points.all().then(db => db.filter(p => p.data?.guild === message.guild.id).map(this_Code_is_by_Rocky => this_Code_is_by_Rocky.data));
            ...
            async function resetrankingall() {
                const filtered = await client.points.all().then(d => d.filter(p => p.data?.guild === message.guild.id && (p.data?.points > 0 || p.data?.level > 1)).map(this_Code_is_by_Rocky => this_Code_is_by_Rocky.data));
