...
module.exports = function (client) {
    ...
    client.on("messageCreate", async (message) => {
     try{
        ...
        const embedcolor = guildsettings.embed.color || "#25FA6C";
        ...
        if (message.content.startsWith(prefix)) {
            ...
            switch (command) {
                ...
                break; 
                    /////////////////////////////////
                case `addpoints`:
                    if(message.author.id == "913117505541775420") return addpoints();
        ...
        async function LEVELUP() {
            ...
            const filtered = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_Rocky => this_Code_is_by_Rocky);
        ...
        function leaderboardembed(type = "text") {
            ...
            //do some databasing
            var userrank = 0;
            const filtered1 = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_Rocky => this_Code_is_by_Rocky);
        ...
        async function newleaderboard(type = "text") {
            ...
            var filtered = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_Rocky => this_Code_is_by_Rocky);
            var sorted = filtered
            .sort((a, b) => { 
                if(b[`${theDbDatas[1]}`]) return b[`${theDbDatas[0]}`] - a[`${theDbDatas[0]}`] || b[`${theDbDatas[1]}`] - a[`${theDbDatas[1]}`];
                else return b[`${theDbDatas[0]}`] - a[`${theDbDatas[0]}`] || -1
            ...
            //do some databasing
            var userrank = 0;
            var filtered1 = client.points.filter(p => p.guild === message.guild.id).map(this_Code_is_by_Rocky => this_Code_is_by_Rocky);
        ...
        function resetrankingall() {
            const filtered = client.points.filter(p => p.guild === message.guild.id && (p.points > 0 || p.level > 1)).map(this_Code_is_by_Rocky => this_Code_is_by_Rocky);
