...
module.exports = client => {
  client.on("interactionCreate", async (interaction) => {
    ...
    if (!member.voice.channel)
      return interaction.reply({
        content: `:no_entry_sign: **Please join a Voice Channel first!**`,
        ephemeral: true
      })
      ...
      if (player && interaction.customId != `Lyrics` && check_if_dj(client, member, player.queue.current)) {
        return interaction.reply({
          embeds: [new MessageEmbed()
            ...
            .setTitle(`:no_entry_sign: **You are not a DJ and not the Song Requester!**`)
      ...
      case "Autoplay": {
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setColor(es.color)
            .setTimestamp()
            .setTitle(`${player.get(`autoplay`) ? `:white_check_mark: **Enabled Autoplay**`: `:no_entry_sign: **Disabled Autoplay**`}`)
            .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({
              dynamic: true
            })))
          ]
      ...
      case "Song": {
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setColor(es.color)
            .setTimestamp()
            .setTitle(`${player.trackRepeat ? `:white_check_mark: **Enabled Song Loop**`: `:no_entry_sign: **Disabled Song Loop**`}`)
            .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({
              dynamic: true
            })))
          ]
      ...
      case "Queue": {
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setColor(es.color)
            .setTimestamp()
            .setTitle(`${player.queueRepeat ? `:white_check_mark: **Enabled Queue Loop**`: `:no_entry_sign: **Disabled Queue Loop**`}`)
            .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({
              dynamic: true
            })))
          ]
        ...
        if(player.volume <= 140){
          ...
          interaction.reply({
            embeds: [new MessageEmbed()
              .setColor(es.wrongcolor)
              .setTimestamp()
              .setTitle(`🔊 **Maximum volume reached!, Volume setted to 150**`)
              .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({
                dynamic: true
               })))
              ]
        ...
        if(player.volume > 10){
          ...
          interaction.reply({
            embeds: [new MessageEmbed()
              .setColor(es.wrongcolor)
              .setTimestamp()
              .setTitle(`🔉 **Minimum volume reached!, Volume set to 1**`)
              .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({
                dynamic: true
              })))
            ]
  ...
  client.on("messageCreate", async message => {
    ...
    if (prefixRegex.test(message.content)) {
      ...
      if (cmd || cmd.length === 0) return // message.reply(":no_entry_sign: **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
      ...
      {
        return // message.reply(":no_entry_sign: **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
      }
    }
    ...
    if (!channel) return message.reply(":no_entry_sign: **Please join a Voice Channel first!**").then(msg => {
      setTimeout(() => {
        try {
          msg.delete().catch(() => {});
        } catch (e) {}
    ...
    if (player && channel.id !== player.voiceChannel) return message.reply(`:no_entry_sign: **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`).then(msg => {
      setTimeout(() => {
        try {
          msg.delete().catch(() => {});
        } catch (e) {}
...
function generateQueueEmbed(client, guildId, leave) {
  ...
  if (!leave && player && player.queue && player.queue.current) {
    embeds[1].setImage(`https://img.youtube.com/vi/${player.queue.current.identifier}/mqdefault.jpg`)
      ...
      .addField(`:sound: Current volume:`, `\`${player.volume}% \``)
  ...
  var songbutton = new MessageButton().setStyle('SUCCESS').setCustomId('Song').setEmoji(`🔁`).setLabel(`Song Loop`).setDisabled();
